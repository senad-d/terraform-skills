...
Create or append embedded AGENTS.md content to the target file.

Behavior:
  - If TARGET_PATH does not exist, this script creates it and writes the full
    embedded contents of AGENTS.md into it.
  - If TARGET_PATH already exists, this script checks whether it already
    contains both "## Memory Bank Structure" and
    "## Project Intelligence (instructions)". If both are found, it skips any
    modification and reports that the content is already present. Otherwise, it
    appends the embedded contents of AGENTS.md starting from line 6 (skipping
...
The `memory-bank/tasks/` folder contains individual markdown files for each task, along with an index file:

- `memory-bank/tasks/_index.md` - Master list of all tasks with IDs, names, and current statuses
- `memory-bank/tasks/TASKID-taskname.md` - Individual files for each task (e.g., `TASK001-implement-login.md`)
